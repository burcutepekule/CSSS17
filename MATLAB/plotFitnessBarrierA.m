...
allX         = reshape(X,length(y0LastVary),N*PN);
binK         = kMat>kFunc(2);
...
indx = 1;
trimVec= [1 4 8 10];
y0LastVaryTrim = y0LastVary(trimVec);
...
for yy=1:length(y0LastVaryTrim)
    subplot(2,2,indx)
    ...
    title(['$m$ : ' num2str(0)  ', $x_{4}^{4}[0]$ : ' num2str(y0LastVaryTrim(yy))], 'interpreter','latex')
    axis([0 N*PN+1 0 1])
    indx = indx+1;
